...
-- Green Resources table
CREATE TABLE IF NOT EXISTS green_resources (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(500) NOT NULL,
    code VARCHAR(100) UNIQUE NOT NULL,
    resource_type VARCHAR(50) NOT NULL,  -- tree, plant, garden, solar_panel, recycling_bin
    zone_id UUID REFERENCES green_zones(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_green_resources_type ON green_resources(resource_type);
CREATE INDEX IF NOT EXISTS idx_green_resources_location ON green_resources USING GIST(location);
...
INSERT INTO green_resources (name, code, resource_type, zone_id, quantity, status, description)
SELECT 
    'Cây xanh khu A', 'GR-001', 'tree', gz.id, 100, 'active', 'Cây xanh khu vực phía Bắc công viên'
FROM green_zones gz WHERE gz.code = 'GZ-001'
ON CONFLICT (code) DO NOTHING;

INSERT INTO green_resources (name, code, resource_type, zone_id, quantity, status, description)
SELECT 
    'Thùng rác tái chế', 'GR-002', 'recycling_bin', gz.id, 20, 'active', 'Thùng rác phân loại'
FROM green_zones gz WHERE gz.code = 'GZ-001'
ON CONFLICT (code) DO NOTHING;
